import PosterBasket from '../assets/img/athletes/posters/Poster-Fer.png';
import PosterSkate from '../assets/img/athletes/posters/Poster-Aleja.png';
import PosterSwim from '../assets/img/athletes/posters/Poster-Sofi.png';

export const ATHLETE = {
  BASKET: 'basket',
  SWIM: 'swim',
  SKATE: 'skate',
};
export const ATHLETHES_VIDEO = {
  [ATHLETE.BASKET]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/Sinergia.mp4?alt=media&token=9f263a01-9475-4fd3-abae-469b5ba7196e',
  [ATHLETE.SWIM]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/Profundo.mp4?alt=media&token=ae59cbc1-46f6-479d-bbe0-f480d213f50d',
  [ATHLETE.SKATE]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/Revancha.mp4?alt=media&token=450bbae3-b991-485e-ac1b-c8e283c925c0',
};
export const ATHLETHES_SOUND = {
  [ATHLETE.BASKET]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/music%2FBasket.mp3?alt=media&token=bfde0dbb-2e47-4681-a0f6-f06aa4f18320',
  [ATHLETE.SWIM]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/music%2FSwim.mp3?alt=media&token=7db4579d-21b8-444c-81ac-89083aff0098',
  [ATHLETE.SKATE]:
    'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/music%2FSkate.mp3?alt=media&token=c264f8c9-b11d-4def-89eb-07b9806dcfd3',
};

export const ATHLETHES_POSTER = {
  [ATHLETE.BASKET]: PosterBasket,
  [ATHLETE.SWIM]: PosterSwim,
  [ATHLETE.SKATE]: PosterSkate,
};

export const AMBIEND_SOUND =
  'https://firebasestorage.googleapis.com/v0/b/desafiandoelequilibrio.appspot.com/o/music%2FAmbient.mp3?alt=media&token=578c197a-8f0d-4a69-8e8f-3e91156b53fa';
